...
const upload = multer({
    storage: multer.diskStorage({
        ...
        filename: (req, file, cb) => {
            // 保留原始文件名，避免重复，采用 Windows 风格冲突命名：name (n).ext
            // 注意：浏览器在 multipart header 中传输的 filename 在 Node 中默认可能按 latin1 解码，
            // 对于包含中文的名字会出现乱码。这里把原始名称按 latin1 -> utf8 重新解码，
            // 以确保中文等非 ASCII 字符正确保存。
...
// 设计说明：所有来自客户端的路径都必须经过此校验，禁止绝对路径或 ".." 越界。
...
app.post('/api/sleep', (req, res) => {
    ...
    exec('rundll32.exe powrprof.dll,SetSuspendState 0,1,0', (error) => {
        res.json({ 
            success: !error, 
            message: error ? `执行失败: ${error.message}` : '睡眠命令已执行' 
        });
    });
});
...
app.post('/api/cloud/search', async (req, res) => {
    try {
        ...
        const types = Array.isArray(req.body.types) ? req.body.types : null; // 可选过滤：['image','video','text','doc']

        if (!isValidPath(userPath)) return res.json({ success: false, message: '无效路径' });

        const startDir = path.resolve(CLOUD_DIR, userPath || '');
        ...
        async function walk(dir) {
            ...
            for (const it of items) {
                ...
                const rel = path.relative(path.resolve(CLOUD_DIR), full).split(path.sep).join('/');
                if (it.isDirectory()) {
                    await walk(full);
                } else {
                    // 过滤类型
                    if (types && types.length > 0) {
                        ...
                        const map = {
                            ...
                            text: ['txt','md','json','xml','log','csv'],
                            doc: ['pdf','doc','docx','epub']
                        };
                        let matchedType = false;
                        for (const t of types) {
                            const arr = map[t];
                            if (arr && arr.includes(ext)) { matchedType = true; break; }
                        }
...
app.get('/api/cloud/preview', async (req, res) => {
    try {
        ...
        if (ext === 'epub') {
            // 为了兼容 epub.js 的分块加载（Range 请求），不要直接全量流式返回，
            // 而是重定向到支持 Range 的下载接口（inline=1），该接口会正确处理 Range
            // 这样 epub.js 可以使用断点/范围请求来获取章节数据，提升大型 epub 的加载体验。
...
// 支持 Range 请求（断点续传、视频 seek）并通过 inline 参数决定是否设置 Content-Disposition
// 使用示例: GET /api/cloud/download?password=xxx&path=some%2Ffile.zip
...
// 支持路径形式的下载 URL，便于 epub.js 使用相对路径时解析为 /api/cloud/download/<file>/...
// 例如: /api/cloud/download/some%2Fbook.epub?password=xxx&inline=1
app.get('/api/cloud/download/*', async (req, res) => {
    try {
